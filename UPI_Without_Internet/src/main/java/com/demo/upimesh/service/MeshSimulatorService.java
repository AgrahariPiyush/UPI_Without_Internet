...
public class MeshSimulatorService {
    ...
    public GossipResult gossipOnce() {
        ...
        // Count how many packet transfers happen in this gossip round.
        int transfers = 0;
        //all devices participating in the mesh network.
        ...
        // Process every device as packet sender.
        for (VirtualDevice src : deviceList) {

            // Read packets from snapshot rather than live state.
            for (MeshPacket pkt : snapshot.get(src.getDeviceId())) {
                if (pkt.getTtl() <= 0) continue;      // Packet expired

                //forwarding packet to every other device
                for (VirtualDevice dst : deviceList) {
                    if (dst == src) continue;    // Don't send packet back to self
                    if (dst.holds(pkt.getPacketId())) continue; //Skip if destination already has packet.

                    // Each device stores its own copy
                    MeshPacket copy = new MeshPacket();
                    copy.setPacketId(pkt.getPacketId());
                    copy.setTtl(pkt.getTtl() - 1);  // Reduce TTL by 1 because packet consumed one hop.
                    copy.setCreatedAt(pkt.getCreatedAt());
                    copy.setCiphertext(pkt.getCiphertext());
                    dst.hold(copy);   // Deliver packet to destination device.
                    transfers++; //suceessfull hopping
                }
            }
        }
    ...
    public List<BridgeUpload> collectBridgeUploads() {
        ...
        //List that will contain all packets ready to be uploaded
        List<BridgeUpload> out = new ArrayList<>();

        //check evry device
        for (VirtualDevice d : devices.values()) {
            if (!d.hasInternet()) continue; //only internet devices

            //devuce has internet : Collect all packets currently stored in it.
